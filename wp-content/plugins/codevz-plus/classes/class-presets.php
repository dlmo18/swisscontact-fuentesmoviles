...
if ( ! class_exists( 'Codevz_Presets' ) ) {
	...
	class Codevz_Presets {
		...
		public function append_presets() {
			...
			foreach ( $this->presets_list() as $n => $v ) {
				...
				vc_add_param( $n, [
					'type' 			=> 'cz_presets',
					'param_name' 	=> $n,
					'group' 		=> esc_html__( 'Presets', 'codevz-plus' ),
					'weight'		=> -1,
		...
		public function presets() {
			...
			$t = Codevz_Plus::_GET( 'type' );

			if ( ! $t ) {

				esc_html_e( 'Not found', 'codevz-plus' );

			} else {

				echo "<link rel='stylesheet' href='" . esc_url( Codevz_Plus::$url ) . "assets/css/codevzplus.css' media='all' />";

				echo "<link rel='stylesheet' href='" . esc_url( Codevz_Plus::$url ) . "wpbakery/assets/css/" . esc_attr( str_replace( 'cz_', '', $t ) ) . ".css' media='all' />";

				if ( Codevz_Plus::$is_rtl ) {
					echo "<link rel='stylesheet' href='" . esc_url( Codevz_Plus::$url ) . "wpbakery/assets/css/" . esc_attr( str_replace( 'cz_', '', $t ) ) . ".rtl.css' media='all' />";
				}

				if ( $t === 'cz_2_buttons' || $t === 'cz_service_box' ) {

					echo "<link rel='stylesheet' href='" . esc_url( Codevz_Plus::$url ) . "wpbakery/assets/css/button.css' media='all' />";

					if ( Codevz_Plus::$is_rtl ) {
						echo "<link rel='stylesheet' href='" . esc_url( Codevz_Plus::$url ) . "wpbakery/assets/css/button.rtl.css' media='all' />";
					}
				...
				foreach( $this->presets_list( $t ) as $p ) {
					...
					// Output
					echo "<div data-num='" . esc_attr( $p['n'] ) . "' data-shortcode='" . wp_json_encode( $v, JSON_HEX_APOS ) . "'>";

					if ( $p['e'] === 'con' ) {
						echo "<div class='cz_pre_in'>" . do_shortcode( $p['s'] ) . "</div>";
					} else {
						echo "<img src='https://xtratheme.com/img/presets/" . esc_attr( $t ) . "_" . esc_attr( $p['n'] ) . "." . esc_attr( $p['e'] ) . "' />";
					}
					echo "</div>";
				}
			}
