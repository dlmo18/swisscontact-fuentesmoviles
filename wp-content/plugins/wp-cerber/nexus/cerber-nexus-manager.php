...
require_once( __DIR__ . '/cerber-nexus-list.php' );

const CRB_ADD_MANAGED_LNK = '#TB_inline?width=450&height=350&inlineId=crb-popup-add-site';
const CRB_NX_MANAGED = 'managed-edit-form'; // Special form for editing managed site data
...
final class CRB_Nexus {
	...
	private static $last_http;
	private static $last_curl;

	/**
	 * Sends request to a remote client website
	 *
	 * @param array $request
	 * @param int $client_id
	 *
	 * @return bool|mixed
	 */
	static function send( $request, $client_id = null ) {

		nexus_diag_log( '/\/ Initiating ' . $request['type'] . ' request to the website' . ( ( $client_id ) ? ' #' . $client_id : ' from context' ) );

		$website = ( $client_id ) ? nexus_get_client_data( $client_id ) : nexus_get_context();

		if ( ! $website ) {
			return false;
		}

		$network = CRB_Nexus::net_send_request( $request, $website );

		nexus_update_client_data( $website->id, array( 'last_http' => self::$last_http ) );

		if ( ! crb_is_wp_error( $network ) ) {

			nexus_process_extra( $network, $website );

			if ( ! empty( $network['payload']['error'] ) ) { // A critical error on the remote website
				$m = 'An error occurred on ' . $website->site_name . ': ' . crb_generic_escape( $network['payload']['error'][1] );
			...
			$causes = array(
				...
				'The IP address of this main website does not match the one set in the access settings on the remote website',
			...
			if ( $network->get_error_code() == 'nexus_http_error' ) {
				if ( ! $error = crb_array_get( $codes, self::$last_http ) ) {
					$desc = self::$last_http . ' ' . get_status_header_desc( self::$last_http );
					$error = '<a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/' . self::$last_http . '" target="_blank">' . $desc . '</a>';
				}
				else {
					$error = self::$last_http . ' ' . $error;
				}
				$error = 'HTTP ERROR ' . $error;
			}
			else {
				$error = crb_escape( $network->get_error_message() );
			}

			nexus_diag_log( 'NETWORK ERROR: ' . $error );

			$parse = parse_url( $website->site_url );
			$domain = $parse['host'];
            ...
            <div style="padding: 4em;">
                ...
                Remote website: <?php echo $website->site_name; ?><br/>
                Remote website URL: <?php echo $website->site_url; ?><br/>
                <p style="font-weight: 600;">This may be caused by a number of reasons</p>
                <ul style="list-style: disc;">
					<?php

					if ( ! $kb_filter = crb_array_get( $kb, $network->get_error_code() ) ) {
						if ( ! $kb_filter = crb_array_get( $kb, self::$last_http ) ) {
							$kb_filter = null;
						}
					}
                ...
                if ( $website = nexus_get_context() ) {
					echo '<p><a href="' . nexus_get_back_link() . '">Switch back to the main website</a></p>';
					echo '<p><a href="' . rtrim( $website->site_url, '/' ) . '/wp-admin/admin.php?page=cerber-nexus" target="_blank">Check the WP Cerber access settings on ' . $website->site_name . '</a></p>';
				}
                ...
                <p class="crb-monospace" style="font-size: 90%">
                    HTTP code: <?php echo self::$last_http; ?><br/>
                    Response size: <?php echo self::$last_curl['size_download']; ?><br/>
                    IP address: <?php echo $ip; ?><br/>
                    Hostname: <?php echo $hostname; ?><br/>
					<?php

					foreach ( self::$last_curl as $key => $val ) {
						if ( is_scalar( $val ) ) {
							echo $key . ': ' . crb_generic_escape( $val );
						}
						else {
							echo $key . ': ';
							print_r( $val );
				...
				if ( $http_content = $network->get_error_data() ) {
					...
					echo '<div class="crb-monospace" style="width: 100%; max-height: 50vh; overflow: auto; background-color: #fff; border: 2px solid #aaa; padding: 1em;">' . crb_generic_escape( $http_content ) . '</div>';
				}
				?>
	 ...
	 */
	static function net_send_request( $payload, $remote ) {
		...
		if ( empty( $remote->site_pass )
		     || empty( $remote->site_url )
		     || empty( $remote->x_field )
		     || empty( $remote->x_num )
		     || ( ! $field_names = nexus_get_fields( $remote ) ) ) {
			return new WP_Error( 'invalid_client_data', 'Mandatory fields for connecting to the managed website #' . $remote->id . ' are invalid or missing' );
		}
		...
		if ( ! cerber_is_wp_ajax()
			...
			$data['at_site'] = ( ! crb_get_settings( 'master_at_site' ) ) ? '' : ' @ ' . $remote->site_name;
		...
		$auth = hash( 'sha512', $remote->site_pass . sha1( $fields[ $field_names[0] ] ) );
		...
		nexus_diag_log( 'Sending HTTP request to ' . $remote->site_url );

		$curl_opt = array(
			CURLOPT_URL               => $remote->site_url,
		...
		crb_configure_curl( $curl, $curl_opt, 'cerber_hub_proxy' );

		$response = @curl_exec( $curl );
		$curl_info = curl_getinfo( $curl );
		$code = intval( curl_getinfo( $curl, CURLINFO_HTTP_CODE ) );
        self::$last_http = $code;
		self::$last_curl = $curl_info;

		curl_close( $curl );

		if ( $code == 200 ) {
			nexus_diag_log( 'HTTP 200 OK' );
			nexus_diag_log( 'Remote data size ' . $curl_info['size_download'] . ', receiving took ' . $curl_info['total_time'] . ' seconds' );
		}
		...
		if ( $code != 200 ) {
			...
			/*if ( $code != 403 ) {
				cerber_update_set( 'bad_response_' . $remote->id, array( time(), $code ) );
			}*/
		...
		if ( ! hash_equals( $ret['echo'], hash( 'sha512', $remote->site_echo . $sha ) ) ) {
			return new WP_Error( 'checksum_error', 'Checksum mismatch: the response we got from the remote website has been altered or secret tokens mismatch.' );
		}

		nexus_diag_log( 'Website "' . $remote->site_name . '" has generated response for ' . $ret['p_time'] );
...
function nexus_show_website_list() {
    ...
    $websites = new CRB_Nexus_Sites();
	$websites->prepare_items();
	$websites->search_box( 'Search', 'search_id' );
	$websites->display();
...
function nexus_show_site_edit_form( $site_id ) {
	$site = nexus_get_client_data( $site_id );
	if ( ! $site ) {
	...
	$edit_fields = nexus_client_form_fields(); // TODO implement this
	...
	cerber_wp_settings_setup( CRB_NX_MANAGED, $edit_fields );
	cerber_show_settings_form( CRB_NX_MANAGED );
}

function nexus_client_form_fields() {
...
add_action( 'admin_init', function () {
	...
	if ( is_admin() && nexus_is_main() ) { // @since 8.6.3.3
		nexus_set_context();
	...
	if ( nexus_is_main() && function_exists( 'nexus_schedule_refresh' ) ) {
	...
	register_setting( 'cerberus-' . CRB_NX_MANAGED, 'cerber-' . CRB_NX_MANAGED );
	add_filter( 'pre_update_option_cerber-' . CRB_NX_MANAGED, function ( $fields, $old_value, $option ) {
		...
		$new_details = array_intersect_key( $fields, array_flip( nexus_client_form_fields() ) );
		$new_details = array_map( 'strip_tags', $new_details );

		$fields = array_replace( $fields, $new_details );

		$fields['group_id'] = $group_id;
		nexus_update_client_data( $site_id, $fields );
...
function nexus_add_client( $token ) {
	if ( ! is_super_admin() || ! nexus_is_main() ) {
		return;
	}
	$token = trim( $token );
...
/**
 * @param $id int Remote site ID
 * @param $data array Sanitized remote site data
 *
 * @return bool
 */
function nexus_update_client_data( $id, $data ) {
	...
	$old = nexus_get_client_data( $id );

	// Details
	$old_details = ( is_array( $old->details ) ) ? $old->details : array();
	$details_fields = nexus_client_form_fields();
	...
	if ( isset( $data['site_name'] ) ) {
		$data['site_name'] = crb_generic_escape( $data['site_name'] );
	}
...
/**
 * Returns remote website data from the local storage
 *
 * @param int $site_id
 *
 * @return bool|object
 */
function nexus_get_client_data( $site_id ) {
...
function nexus_get_clients( $args ) {
	$order = '';
...
/**
 * Deletes client(s) and all remote website data stored locally
 *
 * @param $ids integer|integer[]
 *
 * @return bool
 */
function nexus_delete_client( $ids ) {
	if ( ! is_super_admin() ) {
		return false;
	}
...
function nexus_show_remote_page() {
	...
	if ( cerber_is_http_post()
		 ...
		 && sha1( $client->id . '|' . get_current_user_id() ) === $m ) {
 ...
 * @return bool|mixed
 */
function nexus_send( $request, $client_id = null ) {
	return CRB_Nexus::send( $request, $client_id );
}
 ...
 * @return void
 */
function nexus_process_extra( $data, $client ) {

	$update = array();

	$v = $data['extra']['versions'];

	if ( $client->plugin_v != $v[0] ) {
		$update['plugin_v'] = $v[0];
	}
	if ( $client->wp_v != $v[1] ) {
		$update['wp_v'] = $v[1];
	...
	if ( isset( $v[6] ) && $client->site_key != $v[6] ) {
		$update['site_key'] = $v[6];
	}
	...
	if ( $nums = crb_array_get( $data['payload'], 'numbers' ) ) {
		...
		if ( ! empty( $nums['scan'] ) ) {
			...
			if ( $nb = crb_array_get( $nums['scan'], 'numbers' ) ) {
				...
				cerber_update_set( '_nexus_tmp_' . $client->id, $nb );
			}
		}
        ...
        if ( $plugins = crb_array_get( $nums, 'plugins' ) ) {
			...
			nexus_update_list( $client->id, 'plugins', $installed );
		}
	...
	if ( $update ) {
		nexus_update_client_data( $client->id, $update );
		if ( ! $nums ) {
			nexus_add_bg_refresh( $client->id );
		}
	}
...
function nexus_set_context() {
	...
	if ( $client = nexus_get_client_data( $id ) ) {
		if ( crb_get_settings( 'master_swshow' ) ) {
			cerber_admin_message( sprintf( __( 'You have switched to %s', 'wp-cerber' ), $client->site_name ) . '. ' . 'To switch back to the main website, click the X icon on the toolbar.' );
		}
		...
		cerber_admin_message( __( 'You have switched back to the main website', 'wp-cerber' ) );
		$expire = time();
		$id = 0;
	}
...
function nexus_seal( $check_it = null ) {
	...
	$seal = sha1( $client->id . '|' . $uid . '|' . PHP_VERSION . '|' . PHP_SAPI );
...
function nexus_bg_upgrade( $ids, $plugins ) {
	...
	foreach ( $ids as $id ) {
		$ret = cerber_bg_task_add( 'nexus_do_upgrade', array(
	...
	if ( $error->has_errors() ) {
		crb_admin_error_notice( $error );

        return;
	}

    cerber_admin_message( 'A background upgrade tasks have been launched.' );
}

function nexus_do_upgrade( $client_id, $plugins, $display_errors = false ) {
	$response = CRB_Nexus::send( array(
		'type'    => 'sw_upgrade',
		'sw_type' => 'plugins',
		'list'    => $plugins
	...
	if ( empty( $response ) || ! empty( $response['completed'] ) ) {
		nexus_add_bg_refresh( $client_id );
...
add_action( 'wp_before_admin_bar_render', function () {
	...
	?>
    <div id="crb-popup-add-site" style="display:none;">
        <form id="crb-add-new-site" method="post" action="<?php echo cerber_admin_link( 'nexus_sites' ); ?>"
              style="padding-top:15px; height: 95%;">
	...
	if ( $current ) {
		...
		$wp_admin_bar->add_node( array(
			...
			'meta'  => array( 'class' => 'ab-top-secondary', 'title' => 'Switch to the main website' ),
			'href'  => nexus_get_back_link(),
		) );
	}

	if ( $clients = nexus_get_clients( array( 'orderby' => 'id', 'order' => 'DESC' ) ) ) {
		foreach ( $clients as $client ) {
			...
			$wp_admin_bar->add_node( array(
				...
				'id'     => 'site' . $client->id,
				'title'  => $client->site_name,
				'href'   => $this_page . '&amp;cerber_admin_do=nexus_switch&nexus_site_id=' . $client->id,
			) );
		}
	}
	else {
...
add_action( 'admin_head', function () {
    ...
    <script id="wp-cerber-js-<?php echo crb_sanitize_id( __FUNCTION__ . '_' . __LINE__ ); ?>">
        jQuery(function ($) {
            ...
            $("#wp-admin-bar-crb_managed_site_menu a").attr('target', '_blank');

            $("#crb-nexus-sites .bulkactions .button").on('click', function (e) {
                if ('nexus_delete_slave' === $(this).prev('select').find(':selected').val()) {
                    if (!confirm('<?php echo crb_esc_js( __( 'Are you sure you want to delete selected websites?', 'wp-cerber' ) ); ?>')) {
                        e.preventDefault();
                    }
                }
            });
...
add_action( 'wp_ajax_cerber_master_ajax', function () {
	...
	switch ( crb_array_get( $_REQUEST, 'crb_ajax_do' ) ) {
		case 'nexus_view_updates':
			$client_id = absint( $_REQUEST['managed_site_id'] );

			if ( $list = nexus_get_client_plugins( $client_id ) ) {
				$tbody = '';
				...
				*/
				...
				//cerber_bg_task_add( array( 'func' => 'nexus_send', 'args' => array( array( 'type' => 'hello' ), $client_id ) ) );

				$client = nexus_get_client_data( $client_id );
				$response = array( 'html'   => 'No information available. Refreshed: ' . cerber_auto_date( $client->refreshed ),
...
/**
 * Returns the list of plugins installed on the remote website
 *
 * @param int $client_id
 * @param bool $sort
 * @param bool $active_only
 * @param bool $inc_updates
 *
 * @return array
 */
function nexus_get_client_plugins( $client_id, $sort = true, $active_only = true, $inc_updates = true ) {

    $client_id = absint( $client_id );
	$ac = ( $active_only ) ? ' AND status = "1"' : '';
	$plugins = cerber_db_get_results( 'SELECT * FROM ' . cerber_get_db_prefix() . CERBER_MS_LIST_TABLE . ' lst JOIN ' . cerber_get_db_prefix() . CERBER_SETS_TABLE . ' sts ON (lst.list_item = sts.the_key) WHERE lst.list_key = "plugins" AND lst.site_id = ' . $client_id . ' ' . $ac );
...
/**
 * Refreshes stored locally client web server meta data
 *
 * @param int $client_id
 *
 * @return void
 */
function nexus_refresh_client_servers( $client_id ) {

	if ( ! $client = nexus_get_client_data( $client_id ) ) {
		return;
	}

	$server_host = parse_url( $client->site_url, PHP_URL_HOST );
	...
	if ( $srv_ip != $client->server_id || $srv_country != $client->server_country ) {
		nexus_update_client_data( $client_id, array( 'server_id' => $srv_ip, 'server_country' => $srv_country ) );
	}
...
function nexus_update_list( $site_id, $key, $items = array() ) {
	...
	foreach ( $items as $item ) {
		if ( is_array( $item ) ) {
			$list_item = cerber_db_real_escape( $item[0] );
			$extra     = cerber_db_real_escape( crb_array_get( $item, 1, '' ) );
			$status    = substr( crb_array_get( $item, 2, '' ), 0, 1 );
		}
		else {
			$list_item = cerber_db_real_escape( $item );
			$extra     = '';
			$status    = '0';
		}
