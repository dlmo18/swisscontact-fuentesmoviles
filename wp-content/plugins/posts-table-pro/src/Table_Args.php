...
class Table_Args {
	...
	public static function get_site_defaults() {
		...
		$defaults = apply_filters_deprecated( 'posts_table_user_default_args', [ $defaults ], '2.5.1', 'posts_table_site_default_args' );

		return apply_filters( 'posts_table_site_default_args', $defaults );
	}
	...
	private function parse_args( array $args ) {
		...
		if ( true === $this->links || 'all' === $this->links ) {
			...
			$linkable_columns = apply_filters( 'posts_table_linkable_columns', array_merge( [ 'id', 'author', 'terms', 'tags', 'categories', 'title', 'image' ], $tax ) );
		...
		// Filter post limit
		$this->post_limit = (int) apply_filters_deprecated( 'posts_table_max_posts_limit', [ $this->post_limit, $this ], '2.5.1' );
