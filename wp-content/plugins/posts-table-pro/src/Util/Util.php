...
class Util {
	...
	public static function sanitize_list_and_space( $value ) {
		// Allows any Unicode letter, digit, underscore, hyphen, comma, plus sign, full-stop, colon, percent, forward slash, space and at sign.
		return preg_replace( '/[^\w+\-\/%:,. @]+/u', '', (string) $value );
	}
	...
	public static function is_valid_search_term( $search_term ) {
		...
		$min_length = max( 1, absint( apply_filters( 'posts_table_minimum_search_term_length', 2 ) ) );
	...
	public static function get_language_strings( $post_type ) {
		$single = _x( 'result', 'table results count (single)', 'posts-table-pro' );
		$plural = _x( 'results', 'table results count (multiple)', 'posts-table-pro' );
		...
		if ( $post_type_string ) {
			if ( 'attachment' === $post_type_string ) {
				$single = _x( 'file', 'table attachment count (single)', 'posts-table-pro' );
				$plural = _x( 'files', 'table attachment count (multiple)', 'posts-table-pro' );
			} else {
		 ...
		 */
		$language_strings = array_intersect_key( apply_filters( 'posts_table_language_defaults', $language_strings ), $language_strings );
	...
	public static function get_pages() {
		...
		$options = [ false => __( '— Select —', 'posts-table-pro' ) ];
