...
class Create extends Step {
	...
	public function get_fields_list() {
		...
		$fields = [
			[
				'type'        => 'text',
				'label'       => __( 'Table name', 'posts-table-pro' ),
				'name'        => 'name',
				'description' => __( 'Give your table a friendly name to help you identify it later (e.g. “Posts in the Weddings category”)', 'posts-table-pro' ),
				'value'       => '',
				'placeholder' => __( 'Name', 'posts-table-pro' )
			],
			[
				'type'        => 'select',
				'label'       => __( 'What type of content do you want to display?', 'posts-table-pro' ),
				'name'        => 'content_type',
				'value'       => '',
				'placeholder' => __( 'Name', 'posts-table-pro' ),
	...
	public function get_data( $request ) {
		...
		return $this->send_success_response(
			[
				'values' => [
					'name'         => '',
					'content_type' => isset( $default_options['post_type'] ) ? $default_options['post_type'] : ''
				]
			]
		);
	}
	...
	public function save_data( $request ) {
		...
		if ( empty( $name ) || empty( $content_type ) ) {
			return $this->send_error_response(
				[
					'message' => __( 'Please enter a name for the table and select a content type.', 'posts-table-pro' )
				]
			);
		}
				...
				[
					...
					'settings' => wp_json_encode(
						[
							...
							'rows_per_page' => $default_options['rows_per_page'],
							'post_limit'    => $default_options['post_limit'],
							'search_box'    => $default_options['search_box'],
							'cache'         => $default_options['cache'],
							'button_text'   => $default_options['button_text'],
						]
					)
				]
			);
