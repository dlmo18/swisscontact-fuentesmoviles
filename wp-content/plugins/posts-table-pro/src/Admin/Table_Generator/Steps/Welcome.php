...
class Welcome extends Step {
	...
	public function get_fields_list() {
		$fields = [
			[
				'type'  => 'license',
				'label' => __( 'License key', 'posts-table-pro' ),
				'name'  => 'license',
				'value' => '',
			],
		];
	...
	public function save_data( $request ) {
		...
		if ( ! $license_details ) {
			return $this->send_error_response(
				[
					'message' => __( 'No license details provided. Please enter a license.', 'posts-table-pro' )
				]
			);
		}
		...
		if ( $status !== 'active' ) {
			return $this->send_error_response(
				[
					'message' => __( 'Please validate your license.', 'posts-table-pro' )
				]
			);
		}
