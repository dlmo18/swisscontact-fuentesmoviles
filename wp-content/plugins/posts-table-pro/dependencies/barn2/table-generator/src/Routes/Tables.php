...
class Tables extends Api_Handler
{
    ...
    public function update_table($request)
    {
        ...
        if ($submitted_settings['table_display'] === 'shop_page' && empty($templates_options)) {
            return $this->send_error_response(['message' => __('Please select one or more templates.','posts-table-pro' )]);
        }
    ...
    public function duplicate_table($table_id)
    {
        ...
        if ($rows_affected) {
            ...
            if (\strpos($duplicated_title, __(' - Copy','posts-table-pro' )) !== \false) {
                $duplicated_index = \substr($duplicated_title, \strrpos($duplicated_title, __(' - Copy','posts-table-pro' )) + \strlen(__(' - Copy','posts-table-pro' )) + 1);
                $duplicated_title = \substr($duplicated_title, 0, \strrpos($duplicated_title, __(' - Copy','posts-table-pro' )) + \strlen(__(' - Copy','posts-table-pro' )) + 1) . ' ' . ($duplicated_index == '' ? '2' : $duplicated_index + 1);
            } else {
                $duplicated_title .= __(' - Copy','posts-table-pro' );
            }
