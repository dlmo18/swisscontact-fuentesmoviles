...
namespace Barn2\Plugin\Posts_Table_Pro\Dependencies\Lib\Plugin\Admin;

use Barn2\Plugin\Posts_Table_Pro\Dependencies\Lib\Plugin\License\License_API;
...
class Plugin_Updater implements Registerable
{
    ...
    function auto_update_setting_html($html, $plugin_file)
    {
        ...
        if (!$this->plugin->get_license()->is_valid() || !\apply_filters('barn2_plugin_allow_automatic_update', \true, $this->plugin)) {
            $html = \sprintf('<em>%s</em>', __('Auto-updates unavailable.', 'posts-table-pro'));
        }
        return $html;
    }
    /**
